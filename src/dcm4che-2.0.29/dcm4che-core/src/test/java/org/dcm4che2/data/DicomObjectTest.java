...
public class DicomObjectTest extends TestCase {
    ...
    public void testResolveTagAlreadyExists() throws Exception {
    	DicomObject ds = new BasicDicomObject();
    	int existingTag = 0x7FDB1010;
    	int newTag = 0x7FDB1011;
    	String existingResolve = "existingResolve";
    	String newResolve = "newResolve";
    	int firstRes = ds.resolveTag(existingTag,existingResolve,true);
    	assertEquals("Tag must resolve to the expected value.", existingTag, firstRes);
    	
    	int secondRes = ds.resolveTag(newTag,newResolve,false);
    	assertEquals("Tag without creating it must not create anything", -1, secondRes);
    	int finalRes = ds.resolveTag(newTag,newResolve,true);
    	assertEquals("Secondary resolve should find instance", (newTag & 0xFFFF00FF) | 0x1100, finalRes);
    }
    ...
    /**
     * Tests that a private OB Sequence that is not a private creator data element can be copied to
     * another {@link DicomObject}.
     * @throws Exception An exception is not expected.
     */
    public void testPrivateOBSequenceCanBeCopied() throws Exception {
        final int tag = 0x004710D1;
        final DicomObject dicomObject = new BasicDicomObject();
        final DicomElement element = dicomObject.putFragments(tag, VR.OB, false);
        element.addFragment("Hello World!".getBytes("UTF-8"));

        assertNotNull("Original", dicomObject.get(tag));

        final DicomObject dicomObjectCopy = new BasicDicomObject();
        dicomObject.copyTo(dicomObjectCopy, false);
        assertNotNull("Copy", dicomObjectCopy.get(tag));
    }

    /**
     * Tests that an OB Sequence with a non private tag can be copied to another {@link DicomObject}.
     * @throws Exception An exception is not expected.
     */
    public void testNonPrivateOBSequenceCanBeCopied() throws Exception {
        final int tag = Tag.EncryptedContent;
        final DicomObject dicomObject = new BasicDicomObject();
        final DicomElement element = dicomObject.putFragments(tag, VR.OB, false);
        element.addFragment("Hello World!".getBytes("UTF-8"));
        ...
        final DicomObject dicomObjectCopy = new BasicDicomObject();
        dicomObject.copyTo(dicomObjectCopy);
        assertNotNull("Copy", dicomObjectCopy.get(tag));
    }

    /**
     * Tests that a private OB Element that is not a private creator data element can be copied to
     * another {@link DicomObject}.
     * @throws Exception An exception is not expected.
     */
    public void testPrivateOBElementCanBeCopied() throws Exception {
        final int tag = 0x004710D1;
        final DicomObject dicomObject = new BasicDicomObject();
        dicomObject.putBytes(tag, VR.OB, "Hello World!".getBytes("UTF-8"));
